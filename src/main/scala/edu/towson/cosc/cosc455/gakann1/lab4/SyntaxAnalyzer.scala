...
class SyntaxAnalyzer {
  ...
  // This method implements the BNF rule for a sentence <S> ::= <NP> <Opt(adverbs)> <V> <NP>
  def Sentence() = {
    resetError()
    if (!errorFound) NounPhrase()
    if (!errorFound) Adverbs()
    if (!errorFound) Verb()
    if (!errorFound) NounPhrase()
  }

  // This method implements the BNF rule for a noun phrase <NP> ::= <A><Ad> <N>
  def NounPhrase() = {
  ...
  // This method implements the BNF rule for an adjectives <Ad> ::= happy | mean| hungry | fat
  def Adjective() = {
  ...
  // This method implements the BNF rule for an adjectives <Adv> ::= accidently | quickly| secretly
  def Adverbs() = {
